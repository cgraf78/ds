...
# The shared bootstrap and this link publisher remain compatible with macOS
# system Bash 3.2, but the published ds command uses Bash 4 features. Check the
# executable that `#!/usr/bin/env bash` will resolve before creating any links.
RUNTIME_BASH=$(type -P bash) || {
  printf 'ds: Bash 4 or newer must be first on PATH\n' >&2
  exit 1
}
RUNTIME_BASH_MAJOR=$(
